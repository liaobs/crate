...
public class Planner extends AnalyzedStatementVisitor<Planner.Context, Plan> {
    ...
    private Plan copyToPlan(CopyAnalyzedStatement analysis, Context context) {
        ...
        if (partitionIdent == null) {
            ...
            PartitionName partitionName = PartitionName.fromPartitionIdent(
                    tableInfo.ident().schema(), tableInfo.ident().name(), partitionIdent);
            where = new WhereClause(null, null, ImmutableList.of(partitionName.toString()));
        }
